...
| 名字 | 简介 | 适用场景 | 触发方式 | 路径 |
| --- | --- | --- | --- | --- |
| `forge` | 显式触发的五阶段编码工作流，默认一路自动执行到 stage 5 | 非 trivial 的开发、重构、修 bug、需要中间产物和 review 闭环的任务 | `$forge` / `/forge` | [`skills/forge`](skills/forge) |
...
./install.ps1 -Skill mentor -Target claude -Scope project -ProjectDir C:\path\to\repo
```
