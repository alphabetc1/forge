...
SKILL_NAME="claude-codex-sync"
MODE="copy"
TARGET="both"
CLAUDE_SCOPE="personal"
PROJECT_DIR=""
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_ITEMS=("SKILL.md" "README.md" "agents" "scripts" "claude-codex-sync")

usage() {
  cat <<'EOF'
Install the claude-codex-sync skill into Codex, Claude, or both.
...
Next:
  Codex : start a new session, then invoke with \$claude-codex-sync
  Claude: start a new session, then invoke with /claude-codex-sync
EOF
