...
🧠 一个有状态的长期学习 skill：它支持围绕一个 GitHub repo、一个技术系统或一个知识领域持续学习，把学习进度写进文件、按课程图推进、维护长期记忆，并在每次会话后给出明确的下一步。

## ✨ One-Liner

`mentor` 适合需要跨多轮持续学习的场景，尤其适合系统学习一个 GitHub repo，或者长期学习像 `LLM inference` 这样的领域知识，并持续跟踪 learner 当前进度、薄弱点和待学习目标。

## 🏗️ 架构流程

```mermaid
flowchart TD
    user([学习对象 / repo / 知识领域]) --> init{learning/{topic-slug}/ 存在吗?}
    ...
    update --> next["输出唯一 next action<br/>和待学习目标"]
...
- 🧠 有长期记忆：能持续跟踪 learner 当前进度、已学内容、薄弱点和待学习目标
...
Codex  : $mentor 帮我长期学习大模型推理，监督我当前进度和待学习目标
...
4. 完成一轮讲解 / 诊断 / drill
5. 更新状态和日志
6. 给出一个唯一、可执行的 next action，并同步待学习目标
...
当前默认内置最完整的是 `LLM inference interview prep`，但 `mentor` 本身不局限于面试准备，也适合系统学习 GitHub repo 或其他技术领域。当前内置课程图覆盖：
